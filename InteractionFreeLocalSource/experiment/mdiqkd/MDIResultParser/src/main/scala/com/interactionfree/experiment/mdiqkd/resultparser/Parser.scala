...
class Parser(brokerEndpoint: String, startTime: LocalDateTime, stopTime: LocalDateTime) {
  private val worker = IFWorker(brokerEndpoint)
  private val rangedQberSectionMetas = worker.Storage.range("TDCLocal", startTime.toString, stopTime.toString,
    Map("RecordTime" -> 1, "Data.DataBlockCreationTime" -> 1, "Data.MDIQKDQBER.ChannelMonitorSync" -> 1)
  ).asInstanceOf[List[Map[String, Any]]]
  println(rangedQberSectionMetas.size)
  Range(0, 30).foreach(i => println(rangedQberSectionMetas(i)))

  //  worker.request("Storage", "range", ['TDCLocal',
  //  "2020-05-05 20:28:00.000000", "2020-05-05 20:35:00.000000"
  //  ], {},
