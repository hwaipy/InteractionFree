...
//
class Parser(brokerEndpoint: String, startTime: LocalDateTime, stopTime: LocalDateTime) {
  private val worker = IFWorker(brokerEndpoint, "TDCLocalParserTest")
  Thread.sleep(1000)
  private val qberMetas = worker.Storage.range("TDCLocal", startTime.toString, stopTime.toString,
    Map("Data.DataBlockCreationTime" -> 1, "Data.MDIQKDQBER.ChannelMonitorSync" -> 1)
  ).asInstanceOf[List[Map[String, Any]]]
  private val qberEntries = qberMetas.map(meta => {
    val data = meta("Data").asInstanceOf[Map[String, Any]]
    val mdiqkdQberMeta = data("MDIQKDQBER").asInstanceOf[Map[String, Any]]
    val syncs = mdiqkdQberMeta("ChannelMonitorSync").asInstanceOf[IterableOnce[Any]].iterator.toArray
    new QBEREntry(meta("_id").toString, syncs(0), syncs(1), if (syncs.size > 2) Some(syncs(2)) else None, data("DataBlockCreationTime"))
  })

  val syncs = qberEntries.map(e => e.slowSync).filter(_.isDefined).map(_.get)
  println(syncs)
  println(syncs.drop(1).zip(syncs.dropRight(1)).map(z => (z._2 - z._1) / 1e12))
  ...
  class QBEREntry(val dbID: String, val tdcStart: Long, val tdcStop: Long, val slowSync: Option[Long], val pcTime: Long) {
    //  , val HOMs: Array[Double], val counts: Array[Int], val QBERs: Array[Int]
...
object Parser extends App {
  val parse = new Parser("tcp://127.0.0.1:224", LocalDateTime.of(2020, 5, 5, 22, 22), LocalDateTime.of(2020, 5, 5, 22, 35))
  //  val a = worker.Storage.rangeMeta("TDCLocal", "2020-05-05 20:28:00.000000", "2020-05-05 20:35:00.000000").asInstanceOf[List[String]]
  //  println(a)
  //  println(a.size)
