...
class MessageBuilder private(updateID: Boolean = true) {
  ...
  if (updateID) {
    content += Message.KeyMessageID -> MessageBuilder.MessageIDs.getAndIncrement
  }
...
class MessageException(message: String, cause: Throwable) extends RuntimeException(message, cause) {
